...
static switch_status_t load_recording_server(switch_xml_t xml)
{
  ...
  recording_server_t *existing;
	...
	existing = switch_core_hash_find(globals.recording_servers_hash, recording_server->name);
	if (existing) {
	...
	int paused)
{
  ...
  char *new_sdp;
	...
	rs = switch_core_session_locate(recording->invite_ctx->recording_uuid);
	if (!rs) {
		switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session),
			SWITCH_LOG_ERROR,
	...
	local_sdp =	switch_channel_get_variable(rch, "sip_local_sdp_str");
